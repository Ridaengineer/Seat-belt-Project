...
Test Seatbelt Alerts
    ...
    ${sound_on}=    Is Sound Alarm On
    Should Be True    ${light_red}    The warning light should be red when the seatbelt is not fastened, the engine is running, a person is present, and the speed is over 60 km/h.
    Should Be True    ${sound_on}    The audible alarm must be activated when the seatbelt is not fastened, the engine is switched on, a person is present, and speed exceeds 60 km/h.
...
Test Speed Less Than 60
    [Documentation]    Check that the audible alarm does not sound when vehicle speed is below 60 km/h and the indicator light is yellow.
    Start Engine
    Person Sits
    Unfasten Seatbelt
    ...
    ${sound_on}=    Is Sound Alarm On
    ${sound_off}=  Set Variable    ${sound_on} == False
    Should Be True    ${light_yellow}    The warning light should be yellow when speed is below 60 km/h.
    Should Be True    ${sound_off}    The audible alarm should not be activated at speeds below 60 km/h.

Test All Conditions Met
    [Documentation]    Check that the audible alarm sounds when the seatbelt is not fastened, the engine is running, a person is present and the speed is greater than or equal to 60 km/h.
    Start Engine
    Person Sits
    Unfasten Seatbelt
